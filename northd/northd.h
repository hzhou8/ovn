...
struct tracked_ovn_ports {
    // hzhou: why removing the ls_changes? lsp changes belongs to ls_changes.
    // hzhou: it is better name it tracked_lsp_ports, because LRP change
    // handling must be different from LSP change handling.
    /* tracked created ports.
     * hmapx node data is 'struct ovn_port *' */
    struct hmapx created;
    // hzhou: Why using hmapx instead of list?
