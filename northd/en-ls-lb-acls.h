...
struct ls_lbacls_record {
    ...
    // hzhou: such reference is error-prone. The en-flow directly uses this
    // reference to access other data of the ovn_datapath, but the dependency
    // is not tracked and propagated through this node. For example,
    // ls_lbacls_rec->od->nbs->acls is used in build_acls but ACL changes are
    // not propagated through this node.
...
// hzhou: this seems should be module internal and should move to the .c
