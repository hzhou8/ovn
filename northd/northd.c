...
struct ovn_port {
    ...
    /* Indicate if the port object is directly referenced by any other
     * objects, For incremental processing. */
    bool referenced;
    ...
    NBREC_LOGICAL_SWITCH_TABLE_FOR_EACH_TRACKED (changed_ls,
        ...
        for (size_t j = 0; j < changed_ls->n_ports; ++j) {
            ...
            if (!op) {
                ...
                if (op->referenced) {
                    /* This port is referenced by some other objects. Fallback
                     * to recompute. */
                    goto fail;
                }
 ...
 * add_lflow_to_temp_list. The caller of the ovn_lflow_add_xxx marcros can get
    ...
    struct hmap *pgs, struct hmap *ls_ports)
{
    ...
    NBREC_PORT_GROUP_TABLE_FOR_EACH (nb_pg, nbrec_port_group_table) {
        ...
        for (size_t i = 0; i < nb_pg->n_ports; i++) {
            ...
            /* Now just let I-P handler fallback to recompute.
             * XXX: Add support for I-P for port-group with properly maintained
             * cross-references. */
            op->referenced = true;
