 ...
 * hzhou: LRPs are also synced, should update the comment
                      ...
                      bool *updated)
{
    ...
    // hzhou: the comment didn't tell why are we doing this.
    ...
    struct hmap *ls_ports, struct hmap *lr_ports)
{
    ...
    SBREC_PORT_BINDING_TABLE_FOR_EACH_TRACKED (pb, sbrec_port_binding_table) {
        ...
        if (is_router_port) {
            // hzhou: it looks like we can handle any router port PB updates
            // but in the else branch we are very strict about what switch port
            // PB updates we can handle, which sounds not right because router
            // ports handling are more complex. Either we missed some checks
            // for the router ports, or we were too strict about swith ports.
        ...
        if (sbrec_port_binding_is_new(pb)) {
             ...
             * hzhou: this part needs update for the router port handling.
...
build_lsp_lflows_for_lbnats(struct ovn_port *lsp,
                            struct ovn_port *lrp_peer, // hzhou: this parameter
                                                       // is unnecessary
                            const struct lr_lb_nat_data_record *lr_lbnat_rec,
                            const struct lr_lb_nat_data_table *lr_lbnats,
                            const struct hmap *lr_ports,
                                 ...
                                 struct hmap *lflows)
{
    ...
    HMAPX_FOR_EACH (hmapx_node, &trk_ovn_ports->updated) {
        ...
        if (lsp_is_router(op->nbsp) && op->peer && op->peer->od->nbr) {
            // hzhou: This logic doesn't exist in the below loop for "created".
            // Is it a miss? If so, does it mean there are test coverage
            // missing for LRP I-P?
            const struct lr_lb_nat_data_record *lr_lbnat_rec;
            lr_lbnat_rec = lr_lb_nat_data_table_find_by_index(
